...
class BsDataThread(object):
    def __init__(self):
        ...
        self.refresh_stats_cache_timer = bs.AppTimer(
            8, babase.CallStrict(self.refreshStats), repeat=True
        )
        self.refresh_leaderboard_cache_timer = bs.AppTimer(
            10, babase.CallStrict(self.refreshLeaderboard), repeat=True
        )
...
v = bs.AppTimer(8, babase.CallStrict(BsDataThread))
...
def do_action(action, value):
    if action == "message":
        _babase.pushcall(babase.CallStrict(bs.chatmessage, value),
